	...
	public class TouchGUIEvent : FsmStateAction
	{
		...
		void DoTouchGUIEvent()
		{
			if (Input.touchCount > 0)
			{
				...
				// guiElement = go.GetComponent<Image>() ?? (GUIElement) go.GetComponent<Text>();
				...
				{
					DoTouch(touch, rectTransform);
				}
			}
		}

		void DoTouch(Touch touch, RectTransform rectTransform)
		{
			// Filter by finger ID
			...
			if (fingerId.IsNone || touch.fingerId == fingerId.Value)
			{
				...
				if (RectTransformUtility.RectangleContainsScreenPoint(rectTransform, touchPos, null))
				{
					// First touch?
					...
					DoTouchOffset(touchPos, rectTransform);
		...
		void DoTouchOffset(Vector3 touchPos, RectTransform rectTransform)
		{
			...
			Rect guiRect = new Rect(corners[0].x, corners[0].y,
